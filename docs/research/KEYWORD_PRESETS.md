...
当前公开版本：`2026.09.02.5`。词库包含 **8 个行业包、778 条规则**，源文件在 [`community/keyword-packs/source.json`](../../community/keyword-packs/source.json)，分类补充词也作为普通数据文件公开，不是藏在扩展代码里的黑箱规则。

## 用户侧行为

| 词库包              |    条数 | 命中后的行为                                   |
| ------------------- | ------: | ---------------------------------------------- |
| 黄推 / 成人引流     |     629 | 黄标 + 人工“标记垃圾并拉黑”入口                |
| 投资 / 带单诈骗     |      28 | 同上                                           |
| 加密货币骗局        |      24 | 同上                                           |
| 兼职刷单 / 任务诈骗 |      24 | 同上                                           |
| 贷款 / 解冻诈骗     |      23 | 同上                                           |
...
| 互动诱导            |      16 | 同上                                           |
| 通用营销引流        |      17 | 同上                                           |
| **总计**            | **778** | **从不自动拉黑；可由用户一键拉黑；不上报社区** |

首次安装和 v0.7.4 升级默认开启全部行业包；分类开关直接显示在「设置 → 关键词规则」第一层，不需要先展开。用户可关闭整个包，或点分类标题后关闭、恢复单条规则；自定义词继续只保留在本机。规则同时匹配昵称、账号名、正文和简介。完整短语会在 NFKC 归一化后忽略空格、标点、符号和 emoji；有序分词规则允许词之间插入有限字符，但不会把倒序或缺词文本算作命中。所有黄标都进入当前页面待处理清单，但只有用户点击“标记垃圾并拉黑”或“一键拉黑全部”才会执行 X 拉黑操作。
...
八个分类词包都由本项目统一维护。外部扩展和公开社区数据集只用于观察类别、写法和遗漏面；最终规则必须进入 FeedSieve 自己的行业分类，不在用户界面挂第三方品牌词包。参考数据中的辱骂、违禁药物和明显无关噪声不会因为“外部已有”就自动进入黄推分类。
...
- 不新增“群”“免费”“老师”等与所属行业无关的单个泛词。
- 政治立场、观点、身份、兴趣或任何与垃圾行为无关的词。
- 用户已关闭仍参与匹配的远程规则，或命中后自动拉黑、自动隐藏、自动上报社区的规则。
