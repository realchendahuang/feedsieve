...
当前公开版本：`2026.09.02.3`。词库包含 **8 个行业包、138 条完整短语**，源文件在 [`community/keyword-packs/source.json`](../../community/keyword-packs/source.json)，不是藏在扩展代码里的黑箱规则。

## 用户侧行为

| 词库包              |    条数 | 命中后的行为                                     |
| ------------------- | ------: | ------------------------------------------------ |
| 黄推 / 成人引流     |      27 | 黄标 + 人工“标记垃圾并拉黑”入口                  |
| 投资 / 带单诈骗     |      20 | 同上                                             |
| 加密货币骗局        |      18 | 同上                                             |
| 兼职刷单 / 任务诈骗 |      18 | 同上                                             |
| 贷款 / 解冻诈骗     |      18 | 同上                                             |
...
| 互动诱导            |      13 | 同上                                             |
| 通用营销引流        |      12 | 同上                                             |
| **总计**            | **138** | **从不自动拉黑；可由用户一键拉黑；不上报社区**   |

首次安装默认开启全部行业包；分类开关直接显示在「设置 → 关键词规则」第一层，不需要先展开。用户可关闭整个包，或点分类标题后关闭、恢复单条规则；自定义词继续只保留在本机。规则同时匹配昵称、账号名、正文和简介。所有黄标都进入当前页面待处理清单，但只有用户点击“标记垃圾并拉黑”或“一键拉黑全部”才会执行 X 拉黑操作。
