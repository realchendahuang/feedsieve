...
- **详细描述**（纯文本，直接粘贴）：
  ...
  社区名单（可选，默认开启，可关闭）：
  • 内置社区维护的垃圾账号名单，自动标注
  • 你拉黑的账号匿名贡献给名单：只上报账号名、分类、话术指纹哈希与外链域名，绝不涉及浏览历史
  • 白名单防误伤，抢救票纠正误标

  隐私（一行版）：凭证不出浏览器、推文原文不出设备、只在你主动拉黑后上报匿名摘要。
...
- **权限用途说明**：
  ...
  | 主机权限 `https://feedsieve-api.chendahuang.com/*` | 自有 API：下载经校验的社区名单快照；用户开启贡献时上报匿名摘要 |

- **数据使用勾选**（收集 = 离开设备的数据）：
  - ✅ 网站内容（Website content）——拉黑对象的话术指纹（单向哈希）与外链域名
  - ✅ 用户活动（User activity）——用户主动拉黑的账号与分类
...
3. Popup → 社区名单: "自动贡献" toggle (on by default) controls all community
   reporting; turning it off stops all transmissions. The ⟳ button syncs the
   community snapshot.
...
- Host "https://feedsieve-api.chendahuang.com/*" (our own API, shipped in this
  ...
  and, only when the user blocks someone with contribution enabled, sends an
  anonymous summary: handle, category, one-way content-fingerprint hash,
  external link hostnames, and a salted-hashed random installation ID.
