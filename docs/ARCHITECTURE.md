...
- **天级 · 计分配置（已上线）**：`DETECTOR_CONFIG` 内置兜底在 detector 包；同参数可随签名词库包的 `detector_config` 段热更（R2 词库 15 分钟节奏，同 manifest/checksum/验签链），扩展端严格白名单校验（未知 key / 越界整包拒收），缺省或非法一律回退内置——词库发布即参数热更，无需发版。
