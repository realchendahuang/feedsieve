...
- **结构与昵称双信号**：单词沙拉规则必须佐证账号侧（引流隐语昵称/简介），否则会命中真实「心情贴」
  ...
  佐证通道可随新批次扩展：2026-09-08 乱码昵称批次（`mhmsez ruwzw`，不写引流话术）把佐证扩成
  「引流话术 OR 乱码 handle」，乱码判据用语言学双征（稀有字母 jqvxz ≥2 + 最长辅音串 ≥4，
  真人姓名 handle 至多满足其一）——启发式内已有实现可抄，新批次先复用别另起炉灶。
  佐证优先只依赖 handle（冻结决策的稳定字段），昵称/简介会被懒加载吞掉。
