...
> **Fast local filtering first. Optional AI judgment second.**

Obvious junk should never need a model call. AI is reserved for ambiguous content that rules cannot reliably classify.
...
1. 不要求用户更换 X 客户端。
2. 可以直接清理 Home Timeline、Replies、Search 等原生页面。
3. 垃圾内容可以在进入用户视野前被折叠或隐藏。
4. 本地规则延迟低、成本低、隐私更好。
5. AI 判断可以作为第二层能力按需开启。
6. 安装成本低，天然适合开源传播。

FeedSieve 暂时不需要从第三方 X 客户端开始。
