...
「设置 → 关键词规则」支持添加自己的词或短语；官方词库按黄推、投资诈骗、加密骗局、兼职刷单、贷款诈骗、博彩引流、互动诱导、通用营销引流分成八个行业包，首次安装默认全部开启，分类开关直接显示在第一层。关闭的包完全不参与检测；点分类标题后仍可逐条移除或恢复。当前预置 138 条可审计的完整短语（例如“同城上门约炮”“保本高息”“刷单返利”“空投糖果”），昵称、账号名、正文和简介都会参与匹配，不把“群”“免费”等裸词当规则。词库不随扩展版本冻结：仓库公开源文件经校验后发布到 Cloudflare R2，扩展每 6 小时、启动时或用户手动点击同步时更新，校验失败则保留上一次可信版本。命中只显示黄框；所有黄标都会进入当前页面的一键拉黑清单，但拉黑必须由用户主动点击，自定义词永远只保存在本机。词库的收录依据、发布方式和边界见 [`docs/research/KEYWORD_PRESETS.md`](docs/research/KEYWORD_PRESETS.md)。
