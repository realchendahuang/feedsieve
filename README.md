...
「设置 → 关键词规则」支持添加自己的词或短语；官方词库按黄推、投资诈骗、加密骗局、兼职刷单、贷款诈骗、博彩引流、互动诱导、通用营销引流分成八个可订阅包。用户只订阅自己要清理的类型，未订阅的包完全不参与检测；订阅后还可以逐条移除或恢复。当前预置 126 条可审计的完整短语（例如“保本高息”“内幕消息”“刷单返利”“空投糖果”），不把“群”“免费”等裸词当规则。词库不随扩展版本冻结：仓库公开源文件经校验后发布到 Cloudflare R2，扩展每 6 小时、启动时或用户手动点击同步时更新，校验失败则保留上一次可信版本。命中只显示黄框和人工拉黑入口，绝不自动拉黑或进入一键清理；自定义词永远只保存在本机。词库的收录依据、发布方式和边界见 [`docs/research/KEYWORD_PRESETS.md`](docs/research/KEYWORD_PRESETS.md)。
