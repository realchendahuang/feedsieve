...
describe('buildReportText', () => {
  it('空战报给空态文案（不鼓励硬分享）', () => {
    expect(buildReportText(EMPTY)).toBe('福滤娃今日战报：未发现垃圾账号');
  });
  ...
  it('无分类明细时只输出主句', () => {
    ...
    expect(buildReportText(stat)).toBe('福滤娃今日战报：已拉黑 3 个垃圾账号');
  });
  ...
  it('未知分类被忽略（脏数据不破坏中文战报）', () => {
    ...
    expect(buildReportText(stat)).toBe('福滤娃今日战报：已拉黑 1 个垃圾账号');
  });
});
