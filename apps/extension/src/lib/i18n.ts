...
export const UI_COPY = {
  zh: {
    ...
    communityClean: '社区',
    communityCleanHint:
      '社区确认的垃圾账号名单；拉黑仍由你一键确认，并自动保护关注、白名单和已拉黑账号。',
    cloudEligible: '可一键拉黑',
    cloudProtected: '已排除',
    ...
    queueStarted: (count: number) => `已加入清理队列 ${count} 个`,
    queueRunning: '进行中',
    queuePaused: '已暂停',
    queueCompleted: '已完成',
    queueCancelled: '已取消',
  ...
  en: {
    ...
    communityCleanHint:
      'Shows the community-verified blocklist. You still confirm the block; followed, allowed and already-blocked accounts are protected.',
    ...
    queueStarted: (count: number) => `Queued ${count} for cleanup`,
